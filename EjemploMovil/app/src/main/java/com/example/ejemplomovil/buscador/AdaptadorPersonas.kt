...
import com.example.ejemplomovil.databinding.FragmentoBuscarBinding
...
class AdaptadorPersonas(private var listado: List<DatosUsuario>) :
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var viewsHolder: FragmentoBuscarBinding

        init {
            viewsHolder = FragmentoBuscarBinding.bind(itemView)
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.fragmento_buscar, parent, false)
        )
